...
exports.getList = async params => {
    ...
    const result = await PublicRelationShip.findAndCountAll({
        where: {
            isdel: 0,
        ...
        include: {
            model: VerUnit,
            association: PublicRelationShip.hasOne(VerUnit, {foreignKey:'user_id',sourceKey: 'user_id'}),
        },
        order: [[ 'user_id', 'DESC' ]],
        limit: pageSize,
...
exports.getTarget = async params => {
    ...
    const result = await PublicRelationShip.findOne({ 
        where: {
            isdel: 0,
            user_id,
        },
...
exports.update = async params => {
    ...
    await PublicRelationShip.update(params, {
        where: { user_id },
    });
    return {
        code: 200,
...
exports.create = async params => {
    ...
    const resUnit = await serviceVerUnit.create({ company, admin_id, town, province, legal_person, sub_type: '公' });
    ...
    try {
        ...
        await PublicRelationShip.destroy({
            force: true,
            where: { user_id },
        });
        await PublicRelationShip.create(params);
...
exports.destroy = async params => {
    ...
    await PublicRelationShip.update({
        update_time: TIME(),
...
async function checkCompanyHasExist(company, user_id) {
    ...
    const result = await PublicRelationShip.findOne({ where });
